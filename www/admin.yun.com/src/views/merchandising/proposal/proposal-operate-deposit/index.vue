        ...
        <el-card class="proposal-card-shell" shadow="never">
          ...
          <div class="form-row" style="justify-content: flex-start;margin-top: 16px;">
            <div class="form-item customer">
              ...
              <el-input placeholder="请输入所属客户名称" v-model.trim="customerName"></el-input>
            </div>
            <div class="form-item proposal-name">
              <div class="label">计划书名称</div>
              <el-input placeholder="请输入计划名称" v-model.trim="formData.name"></el-input>
            </div>
          </div>
          <div class="form-row">
            <div class="form-item">
            ...
            <div style="padding: 16px;">
              <el-table
                ...
                <el-table-column
                  ...
                  <template slot="header">
                    <div class="company-box">
                      <i class="iconfont iconxiao_yuanxingguanbi fs14 company-icon-close" @click="handleDelProduct(index)"></i>
                      ...
                      <div class="company-btns">
                        <template v-if="item.is_support_reduction">
                          ...
                          <el-button
                            ...
                            type="primary"
                            size="small"
                            @click="reduceCalc(item)">减保测算</el-button>
                        </template>
                        <el-button type="primary" size="small" plain @click="handleCopyProd(item)">复制</el-button>
                      </div>
                    </div>
                  </template>
                  <template slot-scope="{ row, $index }">
                    <el-select :key="item.colKey" @change="setSexAge(item, index)" v-if="$index === 0" v-model="item.relation" placeholder="请选择被保人">
                      <el-option
                    ...
                    <template v-if="$index === 5">
                      <el-input-number
                        ...
                        placeholder="请输入年交保费"></el-input-number>
                    ...
                    <template v-if="$index === 6">
                      <el-input-number
                        ...
                        placeholder="请输入保费金额"></el-input-number>
                ...
                <el-table-column
                  ...
                  align="center">
                  <template slot="header">
                    <el-button type="primary" icon="el-icon-plus" plain @click="handleAddProduct()">添加产品</el-button>
                  </template>
                </el-table-column>
              </el-table>
              <template>
                <el-radio-group v-model="calcType" size="small" class="mb16 mt16">
                  <el-radio-button :key="key" v-for="(label, key) in calcTypeMap" :label="key">{{label}}</el-radio-button>
                </el-radio-group>
                <el-table
                  ref="cashTable"
                  :data="selYearData"
                      ...
                      <template v-slot="{ $index }">
                        ...
                        <template v-if="item.source === productFrom.MANUALLY_ENTER && calcType === 'cash_value'">
                          <el-input-number
                            ...
                            @change="handleCustCashValChange(item)"
                            v-model="item.cash_value[$index][calcType]"></el-input-number>
                        </template>
  ...
  methods: {
    ...
    previewImage(type) {
      this.submit(type, res => {
        ...
        window.open(`${process.env.API_URL}/deposit-proposal/image/view-by-pcode?pcode=${pcode}`)
      })
    },
    previewPdf(type) {
      this.submit(type, res => {
        const pcode = res.pcode
        window.open(`${process.env.API_URL}/deposit-proposal/pdf/view-by-pcode?pcode=${pcode}`)
      })
    },
    validate() {
