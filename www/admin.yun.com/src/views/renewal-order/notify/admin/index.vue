<template>
  <div class="renewal-notify page-container">
    ...
    <el-scrollbar class="content-body scrollbar" v-loading="loading">
      ...
      <el-switch
        class="mb16"
        v-model="notifyData.is_granted_follow"
        inactive-text="是否授权创富云服协助跟进续保续期"
        :inactive-value="0"
        :active-value="1"
        :active-text="notifyData.is_granted_follow ? '是' : '否'"
    ></el-switch>
      <el-button
        type="primary"
        :loading="submitting"
...
<script>
import { getAdminNotifySetting, setAminNotifySetting } from '@/apis/modules/renewal-order'
export default {
  ...
  created() {
    this.getAdminNotifySetting();
  },
  methods: {
    getAdminNotifySetting() {
      this.loading= true
      getAdminNotifySetting().then(res => {
        this.notifyData = res;
        this.loading =false
      }).catch(() => {
        this.loading =false
    ...
    save() {
      ...
      const params = { 
        is_granted_send_msg: this.notifyData.is_granted_send_msg,  
        is_granted_follow: this.notifyData.is_granted_follow
      }
      setAminNotifySetting(params).then(() => this.$message.success('保存成功!')).finally(() => { this.submitting = false }) 
    }
  }
}
</script>
