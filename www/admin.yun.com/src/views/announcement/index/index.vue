    ...
    <div class="content" v-loading="loading">
        ...
        <div class="unread-filter">
          ...
          >
            ...
            <template v-slot:label
              >{{
                searchModel.read_type
                  ? readMap.find((i) => i.value === searchModel.read_type).label
                  : "全部"
              }}
            </template>
          </filter-shell>
      ...
      <div class="content-wrap" v-if="list && list.length > 0">
        <el-row
          align="middle"
          class="row"
          :class="{ 'is-top': item.is_emphasis, 'is-read': item.is_read != 0 }"
          justify="space-between"
          type="flex"
          @click.native="toDetail(item.announcement_no)"
          v-for="item in list"
          :key="item.announcement_no"
        >
          <el-col :span="22">
            <div class="row-title">
              {{ item.is_top ? "【置顶】" : "" }}{{ item.title }}
            </div>
          </el-col>
      ...
      <div class="table-pagination" v-if="list.length > 0" style="padding-right:30px;">
        <el-pagination
      ...
      <div v-if="!list || list.length == 0" class="tc mt30">暂无数据</div>
    </div>
  </div>
</template>
...
export default {
      ...
      tabData: [
        {
          ...
          dot: "new_lines_quantity",
        },
        {
          name: "regulate",
          label: "商品调整",
          permission: "/home_page/manage",
          dot: "regulate_quantity",
        },
        {
          name: "announcement",
          label: "平台公告",
          permission: "/home_page/manage",
          dot: "announcement_quantity",
        },
      ],
  ...
  methods: {
    ...
    ajaxListData() {
      this.loading = true;
      this.typeMap[this.tabIndex](this.searchModel)
        .then((res) => {
          // this.list =
          //   this.searchModel.page === 1
          //     ? res.list.data
          //     : this.list.concat(res.list.data);
          ...
          this.updateDots({
            ...this.dots,
            [this.unreadTypeMap[this.tabIndex]]: res.un_read_count,
          });
        })
        .catch((err) => {
          console.log(err);
