<template>
  <div class="position-pane-container" ref="container">
    <el-button
      v-if="$checkAuth('/sale/position_and_authority/sales_position/store')"
      class="add-button"
      type="primary"
      @click="posDialogVisible = true"
      size="small"
      ><i class="iconfont iconxiao16_jiahao"></i> 新增职位</el-button
    >
    ...
    <div class="right" v-loading="detailLoading">
      ...
      <div class="content" v-if="selPosVal && curTabIdx === 'people'">
        <div class="desc-wrap flex-between">
          ...
          <div class="flex-center">
            <el-tooltip
              content="职位内无成员才可以删除"
              v-if="
                $checkAuth('/sale/position_and_authority/sales_position/delete')
              "
              placement="top"
            >
              <el-link
                :style="{
                  lineHeight: '20px',
                  color: tableList.length > 0 ? '#999' : null,
                }"
            ...
            <el-button
              ...
              @click="handleSetPos"
              v-if="
                $checkAuth('/sale/position_and_authority/sales_position/update')
              "
              size="small"
        ...
        <div class="table-pagination" v-if="tableList.length > 0">
          <el-pagination
      ...
      <div class="content" v-if="selPosVal && curTabIdx === 'permission'">
        ...
        <div class="flex-center mt10">
          <el-button
            v-if="$checkAuth('/sale/position_and_authority/update')"
            type="primary"
            @click="editTree"
            size="small"
            >编辑权限</el-button
    ...
    <el-dialog
      ...
      width="480px"
      @close="$refs.posForm.resetFields()"
    >
      <el-form
        ref="posForm"
        label-width="100px"
...
export default {
  ...
  methods: {
    ...
    submitModifyPermission() {
      ...
      updatePosTree({ permission_ids, position_id })
        .then(() => {
    ...
    ajaxPositionData(level, id) {
      ...
      getPositionList({ level, role: "sales" })
        .then((res) => {
