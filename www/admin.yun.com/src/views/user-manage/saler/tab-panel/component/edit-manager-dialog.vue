    ...
    <el-dialog custom-class="manager-dialog"
        ...
        <el-form ref="form" :model="formModel" :rules="rules" label-width="100px" label-position="left">
            ...
            <el-form-item label="管理员账号" prop="account_name">
                <el-input placeholder="请输入管理员登录账号" v-model="formModel.account_name"></el-input>
            </el-form-item>
            <el-form-item label="管理员角色" prop="role_id">
                <el-select :loading="selRoleLoading" style="width: 100%" placeholder="请选择管理员角色" v-model="formModel.role_id">
                    <el-option v-for="(item, index) in roleSelData" :key="index" :value="item.id" :label="item.name"></el-option>
                </el-select>
            </el-form-item>
...
<script>
    ...
    import {createManager, editManager, getRoleSelectList } from '@/apis/modules/user-manage'
        ...
        data() {
            return {
                ...
                rules: Object.freeze({
                    ...
                    account_name: [baseValiObj, {validator: this.usernameValidator}],
                    role_id: baseValiObj,
