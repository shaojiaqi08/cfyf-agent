<template>
    <div class="sale-people-container" ref="container">
        <el-button
            ...
            @click="importXS('')"
            size="small"
            ><i class="iconfont iconxiao16_jiahao"></i> 批量导入销售</el-button
        >
        <el-button
            class="add-button"
            type="primary"
            ...
            size="small"
            ><i class="iconfont iconxiao16_jiahao"></i> 新增销售</el-button
        >
        ...
        <div class="right" v-loading="detailLoading">
            <template v-if="selTeam">
                <div class="sale-filter-bar">
                    <div>
                        <filter-shell
                            v-model="searchModel.account_status"
                            :width="250"
                            autoFocus
                            autoClose
                            @input="search"
                        >
                            ...
                            >
                                <el-option
                                    v-for="(item, key) in accountStatusMap"
                                    :key="key"
                                    :label="item.label"
                                    :value="item.value"
                                ></el-option>
                        ...
                        <filter-shell
                            v-model="searchModel.position_id"
                            :width="250"
                            autoFocus
                            autoClose
                            @input="search"
                        >
                            ...
                            <template v-slot:close>
                                <i
                                    ...
                                    @click.stop="searchModel.position_id = ''"
                                ></i>
                            </template>
                        </filter-shell>
                        <filter-shell
                        ...
                        >
                            ...
                            <template v-slot:label>
                                <span>{{
                                    hasValue(resignationDateRange)
                                        ? `${resignationDateRange[0]} ~ ${resignationDateRange[1]}`
                                        : '新增时间'
                                }}</span>
                            </template>
                            <template v-slot:close>
                                <i
                                    ...
                                    @click.stop="resignationDateRange = []"
                                ></i>
                            </template>
                        </filter-shell>
                        <filter-shell
                        ...
                        >
                            ...
                            <template v-slot:label>
                                <span>{{
                                    hasValue(closeDateRange)
                                        ? `${closeDateRange[0]} ~ ${closeDateRange[1]}`
                                        : '注销时间'
                                }}</span>
                            </template>
                            <template v-slot:close>
                ...
                <div v-if="selTeam === -1">
                    ...
                    >
                        ...
                        />
                        <el-table-column
                            v-if="$checkAuth('/sale/set_sales_introducer')"
                            label="推荐人"
                            prop="introduce_sales.real_name"
                            width="150px"
                            align="center"
                        ></el-table-column>
                        <el-table-column
                        ...
                        >
                            <template v-slot="{ row, $index }">
                                <template
                                    v-if="
                                        row.account_status !==
                                        accountStatusMap.cancel.value
                                    "
                                >
                                    <el-link
                                        v-if="$checkAuth('/sale/update')"
                                        type="primary"
                                        class="mr8"
                                        @click="editSales(row.id)"
                                        >编辑</el-link
                                    >
                                    <el-link
                                        v-if="
                                            $checkAuth('/sale/update_password')
                                        "
                                        type="primary"
                                        class="mr8"
                                        @click="modifyPwd(row)"
                                    ...
                                    <el-link
                                        v-if="
                                            $checkAuth('/sale/simulated_login')
                                        "
                                        type="primary"
                                        class="mr8"
                                        @click="genSimulatedLink(row.id)"
                                        >模拟登录</el-link
                                    >
                                    <el-link
                                        v-if="
                                            $checkAuth(
                                                '/sale/update_account_status'
                                            )
                                        "
                                        type="primary"
                                        class="mr8"
                                        ...
                                        >{{
                                            row.account_status ===
                                            accountStatusMap.disable.value
                                                ? '启用'
                                                : '禁用'
                                        }}</el-link
                                    >
                                    <el-link
                                        v-if="$checkAuth('/sale/dimission')"
                                        type="primary"
                                        class="mr8"
                                        @click="dimission(row.id, $index)"
                                        >注销</el-link
                                    >
                                    <el-link
                                        v-if="
                                            $checkAuth(
                                                '/sale/set_sales_introducer'
                                            )
                                        "
                                        type="primary"
                                        class="mr8"
                                        @click="showSetReferrer(row)"
                                        >设置推荐人</el-link
                                    >
                                </template>
                                <span v-else>-</span>
        ...
        <edit-sales-dialog
            ...
            @success="ajaxAllSalesList"
        ></edit-sales-dialog>
        <!--新增团队-->
        <add-team-dialog
            :visible.sync="addTeamDialogVisible"
            :team-data="teamDataWithoutAll"
            :no-team-sales-data="noTeamSalesData"
            @success="ajaxTeamData"
        ></add-team-dialog>
        <!--更换团队主管-->
        <set-leader-dialog
        ...
        <transfer-team-dialog
            ...
            :data="detailData"
            :transfer-team-sel-data="transferTeamSelData"
            :team-id="selTeam"
            @success="refresh"
        ></transfer-team-dialog>
        <!--调整团队成员-->
        <team-people-dialog
            :loading="groupSalesLoading"
            :visible.sync="teamPeopleVisible"
            :data="groupSalesData"
            :selected="groupSelected"
            :submitting="submitting"
            @submit="groupSalesSubmit"
        ></team-people-dialog>
        <!--重置密码-->
        ...
        <!-- 设置推荐人 -->
        <set-referrer-dialog
            v-if="showSetReferrerDialog"
            :visible.sync="showSetReferrerDialog"
            :formModel="curRowInfo"
            @submit="setRefererFun"
        />
    </div>
</template>
<script>
...
import SetReferrerDialog from './component/set-referrer-dialog'
...
export default {
    ...
    computed: {
        filterTableData() {
            ...
            return this.tableData.filter(
                (item) => item.real_name.includes(name) || item.username.includes(name)
            );
        },
        computedTeamData() {
    ...
    methods: {
        ...
        ajaxTransferTeamSelData() {
            ...
            getTransferTeamSelData({ id: this.selTeam })
                .then((res) => {
        ...
        ajaxAllSalesList() {
            ...
            getAllSalesList({ ...this.searchModel, page, page_size })
                .then((res) => {
        ...
        showSetReferrer(row) {
            ...
            data.introduce_sales = data.introduce_sales ? data.introduce_sales : { id: '' }
            this.curRowInfo = data
            this.showSetReferrerDialog = true
        },
        handleSelTeam(obj) {
            this.selTeam = obj.id;
            this.editting = false;
        ...
        dismissTeam() {
            this.confirm(
                "解散团队后，过往业绩将进行归档，且无法恢复原团队，需重新建。是否确认解散？",
                "解散"
            ).then(() => {
        ...
        ajaxDetail(team_id) {
            ...
            getSalesList({ ...searchModel, team_id })
                .then((res) => {
                    this.detailData = res;
                })
                .catch(() => { })
        ...
        ajaxSalesDetail(id) {
            ...
            getSalesDetail({ id })
                .then((res) => {
                    res.resignation_at = formatDate(
                        res.resignation_at * 1000,
                        "yyyy-MM-dd"
                    );
        ...
        ajaxPositionData() {
            ...
            getSalesPositionList({ role: "sales" })
                .then((res) => {
        ...
        ajaxGroupSaleList() {
            ...
            getGroupSalesList({ team_id: this.selTeam })
                .then((res) => {
                    const obj = {};
                    res.forEach((item) => {
        ...
        triggerStatus(row) {
            ...
            this.confirm(content, btnTxt, btnColor, btnClass).then(() => {
                ...
                updateSalesStatus({ id, account_status })
                    .then(() => {
        ...
        setMaxHeight: debounce(function () {
            ...
            if (container) {
                this.maxHeight = this.$refs.container.offsetHeight - 127;
