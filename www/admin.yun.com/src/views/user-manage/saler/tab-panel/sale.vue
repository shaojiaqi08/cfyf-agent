            ...
            >
              ...
              <template v-slot:close>
                <i
                  ...
                  @click.stop="searchModel.account_status = ''"
                ></i>
              </template>
            </filter-shell>
            <filter-shell
                ...
                <span>
                  {{
                    hasValue(searchModel.position_id)
                      ? positionData.find(
                          (i) => i.id === searchModel.position_id
                        ).name
                      : "全部职位"
                  }}
                </span>
        ...
        <div v-if="selTeam === -1">
          ...
          >
            ...
            >
              <template v-slot="{ row }">
                <span>{{
                  formatDate(row.resignation_at * 1000, "yyyy-MM-dd")
                }}</span>
              </template>
            </el-table-column>
            <el-table-column
            ...
            >
              <template v-slot="{ row }">
                <span v-if="row.close_at">{{
                  formatDate(row.close_at * 1000, "yyyy-MM-dd")
                }}</span>
                <span v-else>-</span>
              </template>
            </el-table-column>
            <el-table-column label="状态" prop="join_date" align="center">
              <template v-slot="{ row }">
                <el-tag :type="statusTagType[row.account_status]">{{
            ...
            >
              <template v-slot="{ row, $index }">
                <template
                  v-if="row.account_status !== accountStatusMap.cancel.value"
                >
                  <el-link
                  ...
                  >
                  <el-link
                    v-if="$checkAuth('/sale/update_password')"
                    type="primary"
                    class="mr8"
                    @click="modifyPwd(row)"
                    >重置密码</el-link
                  >
                  <el-link
                    v-if="$checkAuth('/sale/simulated_login')"
                    type="primary"
                    class="mr8"
                  ...
                  >
                  <el-link
                    v-if="$checkAuth('/sale/update_account_status')"
                    type="primary"
                    class="mr8"
                    @click="triggerStatus(row)"
                    >{{
                      row.account_status === accountStatusMap.disable.value
                        ? "启用"
                        : "禁用"
                    }}</el-link
                  >
          ...
          <!-- 20211119 修改分页方式 去除无限滚动 -->
            <div class="table-pagination" v-if="allSalesData.length > 0">
                <el-pagination
          ...
          <el-scrollbar>
            <div class="team-info">
              <div class="flex-column">
                <div class="name-wrap">
                  <template v-if="!editting">
                    ...
                    <el-link
                      v-if="
                        $checkAuth('/sale/team/modify_name') &&
              ...
              <div class="flex-center">
                ...
                <el-button
                  v-if="
                    $checkAuth('/sale/team/change_team_parent') &&
                      detailData.parent_id
                  "
                  type="primary"
                  @click="handleSetTeam"
            ...
            <div class="table-wrap">
              <div class="flex-between table-header">
                <span
                  >当前团队主管人数：{{
                    detailData.leader ? detailData.leader.length : 0
                  }}
                  人</span
                >
                <el-button
            ...
            <div class="table-wrap">
              <div class="flex-between table-header">
                <span
                  >当前团队成员人数：{{
                    detailData.member ? detailData.member.length : 0
                  }}
                  人</span
                >
                <el-button
