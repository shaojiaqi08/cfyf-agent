    ...
    <div class="header">
      <el-tabs class="tabs" v-model="tabIndex" @tab-click="tabChange">
        ...
        <el-tab-pane
          name="rank"
          label="业绩排行"
          v-if="$checkAuth('/team_performance/personal_rank')"
        ></el-tab-pane>
        <el-tab-pane
          name="statistics"
          label="商品统计"
          v-if="
            $checkAuth('/team_performance/insurance_class_rank') ||
              $checkAuth('/team_performance/product_rank')
          "
        ></el-tab-pane>
      </el-tabs>
      <el-input
    ...
    <div class="scroll-box p16" v-if="tabIndex === 'order'" ref="content">
          ...
          <span>
            {{
              hasValue(searchModel.manpower_underwriting_status)
                ? manualReview.find(
                    (i) => i.value === searchModel.manpower_underwriting_status
                  ).label
                : "人核状态"
            }}
          </span>
      ...
      >
        ...
        <el-table-column
          ...
          align="center"
          width="100px"
          v-if="$checkAuth('/team_performance/company_commission')"
        ></el-table-column>
        <el-table-column
          label="佣金(元)"
          ...
          align="center"
          width="100px"
          v-if="$checkAuth('/team_performance/sales_commission')"
        ></el-table-column>
        <el-table-column
          label="操作"
          prop
        ...
        >
          <template slot-scope="{ row }">
            <el-link
              v-if="$checkAuth('/team_performance/detail')"
              type="primary"
...
export default {
  ...
  methods: {
    ...
    policyExport() {
      const url = `${exportTeamPolicy}?${qs.stringify({
    ...
    showInfoDialog(row) {
      ...
      let routeUrl = this.$router.resolve(
        `/achievement-team/detail/${row.order_no}`
      );
    ...
    getTeamPolicyList() {
      ...
      getTeamPolicyList({ ...this.searchModelFormat(), page, page_size })
        .then((res) => {
    ...
    getTeamPolicyStatistics() {
      getTeamPolicyStatistics(this.searchModelFormat())
        .then((res) => {
