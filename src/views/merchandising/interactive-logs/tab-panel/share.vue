<template>
    <div class="share-container">
        <el-input v-model="otherModel.keyword" prefix-icon="el-icon-search" class="search-input" placeholder="搜索产品名称"></el-input>
                        ...
                        <div v-infinite-scroll="nextPage" :infinite-scroll-delay="400">
                            <div v-for="(item,index) in otherDateArr"
                                ...
                                <div class="item pointer"
                                     v-for="it in otherData[item]" :key="it.original_id"
                                    ...
                                    <div class="fs16 word-break mb12 bold">{{it.target_name}}</div>
                                    <div class="gray active-vague"
                                         v-if="it.type === 'evaluating' || it.type === 'drop_game'">
                                        共有{{it.uv_read_count}}位微信好友浏览，{{it.finished_evaluating_count}}人完成{{it.type === 'drop_game' ? '答题':
                                        '测评'}}
                                    </div>
                                    <div class="gray active-vague" v-else>共有{{it.uv_read_count}}位微信好友浏览，平均时长{{it.pv_read_time_text}}</div>
                                </div>
                            </div>
                        </div>
                    </el-scrollbar>
            ...
            <div v-if="otherDetail.caption" class="detail-warp">
                ...
                <div class="detail-content border-bottom">
                    <div class="mb12" v-if="otherDetail.caption.tags.length > 0">
                        <el-tag type="minor" :class="{'mr10': index !== otherDetail.caption.tags.length-1}"
                                v-for="(item,index) in otherDetail.caption.tags"
                                :key="index">{{item}}
                        </el-tag>
                    </div>
                    <div class="fs16 word-break mb12 bold" v-if="otherDetail.caption.type === 'cps'">
                    ...
                    <div class="gray"
                         v-if="otherDetail.caption.type === 'evaluating' || otherDetail.caption.type === 'drop_game'">
                        共有{{otherDetail.caption.uv_read_count}}位微信好友浏览，{{otherDetail.caption.finished_evaluating_count}}人完成{{otherDetail.caption.type
                        === 'drop_game' ? '答题' : '测评'}}
                    </div>
                    <div class="gray" v-else>共有{{otherDetail.caption.uv_read_count}}位微信好友浏览，平均时长{{
                        otherDetail.caption.pv_read_time_text}}
                    </div>
                </div>
                <template v-if="otherDetail.caption.type === 'evaluating' || otherDetail.caption.type === 'drop_game'">
                    <div class="test-friend">
                        <el-scrollbar class="scroll-bar" wrapClass="scroll-bar-wap-friend">
                            <div class="scroll-bar-friend-item">
                                <div class="time border-bottom" :class="{'fixed':friendIndex === 0 }"><span>完成测评的好友</span></div>
                                <template v-if="otherDetail['finished'].length > 0">
                                    ...
                                    <div class="list-user"
                                         ...
                                         v-for="(item,index) in otherDetail.finished" :key="index">
                                        <div class="flex-between">
                                            <el-avatar :size="50" :src="item.user_avatar"></el-avatar>
                                            <div class="flex-item ml10">
                                                <div class="flex-between mb10">
                                                    <span class="fs14 text-ellipsis">{{item.user_nickname}}</span>
                                                    <span class="gray active-vague fs12">{{item.read_date_time_text}}</span>
                                                </div>
                                                <div class="flex-between fs12">
                        <span
                                v-if="otherDetail.caption.type !== 'drop_game'"
                                class="gray active-vague">{{item.has_complete_info === 0 ? '未填写姓名和手机号' : item.user_name + ' '+item.user_mobile}}</span>
                                                    <span v-else class="gray active-vague">{{item.evaluating_result_str}}</span>
                                                    <span class="gray active-vague">{{item.read_minute_time_text}}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <div class="list-no-finish-user"
                                            ...
                                            <div class="flex-between">
                                                ...
                                                <div class="flex-item ml10">
                                                    <div class="flex-between mb10">
                                                        <span class="fs14 text-ellipsis">{{item.user_nickname}}</span>
                                                        <span class="gray fs12">{{item.read_date_time_text}}</span>
                                                    </div>
                                                    <div class="flex-between fs12">
                                                        <span v-if="otherDetail.caption.type !== 'drop_game'" class="gray">{{item.user_name + ' '+item.user_mobile}}</span>
                                                        <span class="gray" v-else>{{item.evaluating_result_str}}</span>
                ...
                <template v-else>
                    <div class="detail-personnel" v-if="otherDetail.users && otherDetail.users.length > 0">
                        <el-scrollbar
                                class="scroll-bar">
                            <div class="list-personnel">
                                <div class="flex-between"
                                     :class="{'mb20': index !== otherDetail.users.length-1}"
                                     v-for="(item,index) in otherDetail.users" :key="index">
                                    <el-avatar :size="50" :src="item.user_avatar || DefaultAvatar"></el-avatar>
                                    <div class="flex-item ml10">
                                        <div class="flex-between mb10">
                                            <span class="fs14 text-ellipsis">{{item.user_nickname}}</span>
                                            <span class="gray fs12">{{item.read_date_time_text}}</span>
                                        ...
                                        <div class="flex-between fs12">
                                            <span class="gray">浏览时长：{{item.read_time_text}}</span>
                                            <span class="gray">{{item.read_minute_time_text}}</span>
                                        </div>
                                    </div>
                                </div>
        ...
        <div class="iframe">
            <div class="content">
                <div class="iframe-url" v-loading="detailLoading">
                    <template
                            v-if="(otherDetail.caption && otherDetail.caption.type === 'evaluating') ||  (otherDetail.caption && otherDetail.caption.type === 'drop_game')">
                        <iframe
                                v-if="activeFinishUrl"
                                class="iframe-item"
                                scrolling="yes" frameBorder=0
                                :src="activeFinishUrl"></iframe>
                        <div class="no-content gray" v-else>
                            <div class="tc">
                                <i class="el-icon-view mb5 fs18"></i>
                                <div>点击列表查看内容详情</div>
                    ...
                    <template v-else-if="otherDetail.caption && otherDetail.caption.type === 'article'">
                        <iframe
                                class="iframe-item"
                                scrolling="yes" frameBorder=0
                                :src="otherDetail.caption.target_url"></iframe>
                    </template>
                    <template v-else>
                        <div class="no-content gray">
                            <div class="tc">
                                <i class="el-icon-view mb5 fs18"></i>
                                <div
                                        v-if="otherDetail.caption && (otherDetail.caption.type === 'cps' || otherDetail.caption.type === 'product')">
                                    请在手机端查看产品详情
                                </div>
...
<script>
    ...
    export default {
        ...
        beforeDestroy() {
            ...
            if (scrollBarOther) {
                scrollBarOther.removeEventListener('scroll', this.scrollBarWapOtherFn, false)
            }
            let scrollBarFriend = document.querySelector('.scroll-bar-wap-friend')
            if (scrollBarFriend) {
                scrollBarFriend.removeEventListener('scroll', this.scrollBarWapFriendFn, false)
            }
        },
        methods: {
            nextPage() {
            ...
            getInteractiveLogOtherArticle(id) {
                ...
                interactiveLogOtherArticle({id: id}).then(res => {
            ...
            getInteractiveLogOtherApi(id) {
                if (!id) return
                this.detailLoading = true
                interactiveLogOtherApi({id: id}).then(res => {
            ...
            getInteractiveLogOtherCps(id) {
                if (!id) return
                this.detailLoading = true
                interactiveLogOtherCps({id: id}).then(res => {
            ...
            getInteractiveLogOtherPit(id) {
                if (!id) return
                this.detailLoading = true
                interactiveLogOtherPit({id: id}).then(res => {
                    console.log('避坑游戏详情', res)
                    res.caption.tags = res.caption.tags.split(',')
                    res.finished = res['users'].filter(item => item.finished_evaluating === 1)
                    res.noFinished = res['users'].filter(item => item.finished_evaluating === 0)
                    console.log(res)
                    this.otherDetail = res
                    this.detailLoading = false
            ...
            getInteractiveLogOtherRisk(id) {
                if (!id) return
                this.detailLoading = true
                interactiveLogOtherRisk({id: id}).then(res => {
            ...
            initScrollOther() {
                ...
                scrollBarOther.addEventListener('scroll', this.scrollBarWapOtherFn, false)
            },
            scrollBarWapOtherFn(e) {
                ...
                for (let i = 0; i < this.offsetTopOtherArr.length; i++) {
                    if (this.offsetTopOtherArr[i + 1]) {
                        if (scrollTop > this.offsetTopOtherArr[i] && scrollTop < this.offsetTopOtherArr[i + 1]) {
            ...
            initScrollFriend() {
                ...
                console.log(offsetTopArr)
                scrollBarFriend.addEventListener('scroll', this.scrollBarWapFriendFn, false)
            },
            scrollBarWapFriendFn(e) {
                ...
                for (let i = 0; i < this.offsetTopFriendArr.length; i++) {
                    if (this.offsetTopFriendArr[i + 1]) {
                        if (scrollTop > this.offsetTopFriendArr[i] && scrollTop < this.offsetTopFriendArr[i + 1]) {
                            this.friendIndex = i
                            return
                        }
                    } else {
